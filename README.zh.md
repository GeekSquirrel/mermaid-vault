...
2. **生产镜像构建**：每次运行都会完整构建单容器生产镜像，确保前端构建或 Dockerfile 的破坏性变更在合并前就能被发现。发布前会对镜像做冒烟测试：容器必须能启动、响应 `/health`、托管前端静态页面，并通过一轮真实的 SQLite API 增删查验证。随后 push 事件将镜像发布到 GHCR（`ghcr.io/geeksquirrel/mermaid-vault`），标签规则如下：
