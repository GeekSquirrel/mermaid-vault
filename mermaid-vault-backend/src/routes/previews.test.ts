...
describe("Preview API Integration Tests", () => {
  it("should round-trip a diagram preview and invalidate it when code changes", async () => {
    const code = "graph LR\n  A[Start] --> B[End]";
    const createRes = await request(app)
      .post("/api/diagrams")
      .send({ title: "Preview Test Diagram", code });
    ...
    const missingRes = await request(app).get(
      `/api/diagrams/${diagramId}/preview.svg?theme=light`
    );
    expect(missingRes.status).toBe(404);
    expect(missingRes.body.success).toBe(false);

    // Upload preview for both themes
    const putLight = await request(app).put(`/api/diagrams/${diagramId}/preview`).send({
      theme: "light",
      codeHash: hash,
      svg: SAMPLE_SVG,
    });
    ...
    const putDark = await request(app).put(`/api/diagrams/${diagramId}/preview`).send({
      theme: "dark",
      codeHash: hash,
      svg: SAMPLE_SVG,
    });
    ...
    const getLight = await request(app).get(`/api/diagrams/${diagramId}/preview.svg?theme=light`);
    ...
    const getDark = await request(app).get(`/api/diagrams/${diagramId}/preview.svg?theme=dark`);
    ...
    const notModified = await request(app)
      .get(`/api/diagrams/${diagramId}/preview.svg?theme=light`)
      .set("If-None-Match", `"${hash}"`);
    expect(notModified.status).toBe(304);

    // Code update invalidates the stored preview
    const newCode = "graph TD\n  Step1 --> Step2";
    await request(app).put(`/api/diagrams/${diagramId}`).send({ code: newCode });
    const staleRes = await request(app).get(`/api/diagrams/${diagramId}/preview.svg?theme=light`);
    expect(staleRes.status).toBe(404);

    // Uploading a preview rendered from the old code is rejected
    const stalePut = await request(app).put(`/api/diagrams/${diagramId}/preview`).send({
      theme: "light",
      codeHash: hash,
      svg: SAMPLE_SVG,
    });
    ...
    const freshPut = await request(app).put(`/api/diagrams/${diagramId}/preview`).send({
      theme: "light",
      codeHash: sha256(newCode),
      svg: SAMPLE_SVG,
    });
    expect(freshPut.status).toBe(200);
    const refreshed = await request(app).get(`/api/diagrams/${diagramId}/preview.svg?theme=light`);
  ...
  it("should validate preview upload inputs", async () => {
    const createRes = await request(app)
      .post("/api/diagrams")
      .send({ title: "Validation Diagram", code: "graph TD; A-->B" });
    const diagramId = createRes.body.data.id;

    const badTheme = await request(app).put(`/api/diagrams/${diagramId}/preview`).send({
      theme: "blue",
    ...
    const badHash = await request(app).put(`/api/diagrams/${diagramId}/preview`).send({
      theme: "light",
      codeHash: "not-a-hash",
      svg: SAMPLE_SVG,
    });
    ...
    const emptySvg = await request(app).put(`/api/diagrams/${diagramId}/preview`).send({
      theme: "light",
      codeHash: sha256("graph TD; A-->B"),
      svg: "",
    });
    ...
    const badThemeQuery = await request(app).get(
      `/api/diagrams/${diagramId}/preview.svg?theme=sepia`
    );
  ...
  it("should return 404 for previews of non-existent resources", async () => {
    const res = await request(app).get("/api/diagrams/no-such-id/preview.svg?theme=light");
