...
### 3.1 前后端联调
- [ ] **3.1.1** 启动后端：`cd backend && bun run dev`。
- [ ] **3.1.2** 启动前端：`cd mermaid-live-editor && npm run dev`。
- [ ] **3.1.3** 打开浏览器控制台，确认无 CORS 报错，所有请求成功。

### 3.2 功能测试用例（手动执行）
- [ ] **3.2.1** 项目列表：首次打开显示空列表（或已有数据），界面布局正常。
- [ ] **3.2.2** 新建项目：点击“新建”跳转编辑器，编写代码，等待自动保存，观察提示变为“已保存”，URL 出现 `projectId`。
- [ ] **3.2.3** 返回列表：新项目出现在列表顶部，标题正确，时间正确。
- [ ] **3.2.4** 打开已有项目：点击列表中的“打开”，跳转编辑器，内容与保存时一致。
- [ ] **3.2.5** 编辑更新：修改代码，等待保存，刷新页面，内容保持最新。
- [ ] **3.2.6** 删除项目：点击“删除”，确认后列表移除，尝试通过 `projectId` URL 直接访问应显示 404 或新建。
- [ ] **3.2.7** 搜索过滤：输入关键字，列表正确过滤（不区分大小写）。
- [ ] **3.2.8** 刷新测试：在编辑器页面刷新，应从 API 重新加载内容，且 `projectId` 保留在 URL 中。

### 3.3 异常与边界测试
- [ ] **3.3.1** 后端未启动时，前端应显示“网络错误”或“连接失败”的友好提示，不崩溃。
- [ ] **3.3.2** 删除项目后，若编辑器中仍打开该项目，再次保存应返回 404 并提示用户（或自动创建新项目）。
- [ ] **3.3.3** 数据库文件不可写时，后端应返回 500 错误，前端捕获并提示。
- [ ] **3.3.4** 输入超长标题/代码，确认数据库无截断或报错（SQLite 限制通常足够，但需确认）。

### 3.4 文档与部署准备
- [ ] **3.4.1** 在根目录更新 `README.md`，说明整体架构（前后端分离）、启动步骤、环境变量配置。
- [ ] **3.4.2** 在 `backend/` 下创建 `README.md`，详细说明 API 接口文档（路径、方法、参数、响应示例）。
- [ ] **3.4.3** 在 `backend/` 下编写 `Dockerfile`（可选，但推荐提供），用于容器化部署。
- [ ] **3.4.4** 前端构建测试：`npm run build` 确保 TypeScript 无报错。
- [ ] **3.4.5** 提交代码：将子模块前端改动提交到你的 fork 仓库，后端代码提交到主仓库，并打上 `v1.0.0` 标签（可选）。

---

## 验收标准（Definition of Done）
- [ ] 后端所有 API 可正常响应，SQLite 数据持久化，迁移自动运行。
- [ ] 前端项目列表可正常展示所有项目，支持新建、打开、删除、搜索。
- [ ] 编辑器完全替换 localStorage 为后端 API，保存状态可见。
- [ ] 多设备切换（如另一浏览器）可看到相同项目列表，内容一致（单用户场景）。
- [ ] 无阻塞性 Bug，控制台无报错，CORS 正常。
- [ ] 根目录和 backend 目录均有完善的 README 文档。
...
**开发执行建议**：按 `M0 → M1 → M2 → M3` 顺序推进，每完成一个小节勾选对应 Checkbox。遇到阻塞时优先参考官方文档（Bun、React Router）。完成 M3 后，整体功能即可交付使用。
